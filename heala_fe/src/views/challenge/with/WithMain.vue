      ...
      <el-col :span="24">
        <el-row>
          ...
          <el-col :span="8" class="container-box-detail">
            <router-link router-link :to="{ name: 'WithMain' }" class="text-decoration-none">[]식단</router-link> 
          </el-col>
          <el-col :span="8" class="container-box-detail">
            <router-link router-link :to="{ name: 'WithMain' }" class="text-decoration-none">[]마음</router-link> 
          </el-col>
        </el-row>
        <el-col class="community-box">
          <el-col :span="24" v-for="(with_challenge, idx) in with_challenges" :key="idx" class="community-inside ">
            <div @click="SetCurrentPageId(with_challenge.id)">
              <router-link :to="{ name: 'WithDetail' , params: with_challenge.id }" class="text-decoration-none">
...
export default {
  ...
  mutations: {
    SETCURRENTPAGEID: function (state, getId) {
      state.currentPageId = getId
      console.log('클릭 감지')
    },
  },
  methods: {
    SetCurrentPageId: function ({ commit }, getId) {
      console.log('클릭 감지1')
      console.log(this.store)
      console.log(this.store.currentPageId)
      commit("SETCURRENTPAGEID", getId)
      console.log('클릭 감지2')
    },
  },
  computed: {
