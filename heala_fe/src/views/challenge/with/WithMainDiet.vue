      ...
      <el-col :span="24">
        ...
        <el-col class="community-box">
          ...
          <el-col :span="24" v-for="(value, idx) in withChallenges" :key="idx" class="community-inside ">
            <!-- {{ value }} -->
            <div v-if="value.withChallenge.category == 1">
...
export default {
  ...
  methods: {
    ...
    getWithDietChallenge: function () {
      axios.get(`${SERVER.URL}${SERVER.ROUTES.getWithDietChallenge}`)
        .then((res) => {
  ...
  mounted: function () {
    this.getWithDietChallenge()
    this.$store.state.selectedRouter = 3
    this.$store.state.currentPageCategory = 1
  },
}
</script>
