<template>
  <div>
    <el-row class="justify-content-space-between">
      ...
      <el-col :span="6">
        <router-link :to="{ name: 'Health' }" class="text-decoration-none selected-category">운동</router-link> 
        <div id="square"></div>
      </el-col>
      <el-col :span="6">
...
export default {
  ...
  methods: {
    async getCommunityHealthInfo() {
      try {
        const response = await axios.get(`${SERVER.URL}${SERVER.ROUTES.community}category?category=${0}&page=${this.limit}&subCategory=${this.$store.state.selectedSubCategory}`)
